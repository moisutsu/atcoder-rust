...
fn main() {
    ...
    let moves = vec![(1, 0), (0, 1), (!0, 0), (0, !0)];
    let mut dist = vec![vec![-1; W]; H];
    let mut q = VecDeque::new();
    let s = {
        let mut s = (0, 0);
        for y in 0..H {
            if let Some(x) = cs[y].get_index(&'s') {
                s = (x, y);
            }
        }
        s
    ...
    while let Some(cur) = q.pop_front() {
        for &(move_x, move_y) in &moves {
            let p = (cur.0.wrapping_add(move_x), cur.1.wrapping_add(move_y));
            if p.0 < W && p.1 < H {
                if cs[p.1][p.0] == 'g' {
                    echo!("YES");
                    return;
                }
                let cur_rest = dist[cur.1][cur.0];
                let p_rest = if cs[p.1][p.0] == '#' {
                    cur_rest - 1
                } else {
                    cur_rest
                };
